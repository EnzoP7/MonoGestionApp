...
import { useActualizarEgreso } from "@/lib/react-query/mutations/egresos/useActualizarEgreso";
...
export function EditEgresoModal({ userId, egreso, isOpen, onClose }: Props) {
  ...
  const updateMutation = useActualizarEgreso({
    userId,
    onSuccess: () => {
      onClose();
    },
  ...
  // Llenar el formulario cuando cambie el egreso
  useEffect(() => {
    if (egreso) {
      setForm({
        ...
        categoriaEgresoId: egreso.categoriaEgresoId || "",
      });
    }
  }, [egreso]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(value) => handleChange("categoriaEgresoId", value === "none" ? "" : value)}
            >
              <SelectTrigger>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={updateMutation.isPending}>
              {updateMutation.isPending ? "Actualizando..." : "Actualizar Egreso"}
            </Button>
          </div>
        </form>
      </DialogContent>
