...
export const getVentasPorDia = unstable_cache(
  async (userId: string, dias: number = 90) => {
  ...
  (userId: string, dias: number = 90) => [`ventas-por-dia:${userId}:${dias}`],
  {
    tags: (userId: string, dias: number) => [`ventas:${userId}`],
  }
);
